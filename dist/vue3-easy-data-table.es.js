...
function useExpandableRow(items, prevPageEndIndex, itemsExpanded, emits) {
  const expandingItemIndexList = ref([]);
  const getItemIndex = (expandingItem) => {
...
const _withScopeId = (n) => (pushScopeId("data-v-03558ea3"), n = n(), popScopeId(), n);
const _hoisted_1 = ["id"];
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose, emit: emits }) {
    const props = __props;
    useCssVars((_ctx) => ({
      "b5b054ea": unref(tableMinHeightPx),
      "4a724b76": unref(tableHeightPx)
    }));
    const {
      tableNodeId,
      clickEventType,
    ...
    const {
      ...
      updateExpandingItemIndexList,
      clearExpandingItemIndexList
    } = useExpandableRow(pageItems, prevPageEndIndex, itemsExpanded, emits);
    const {
      fixedHeaders,
      lastFixedColumn,
      fixedColumnsInfos
...
var DataTable = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-03558ea3"]]);
